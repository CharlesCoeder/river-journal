/**
 * streak.unlock.e2e.test.ts — Story 2.9 (Model B unlock tokens) E2E tests.
 *
 * Covers:
 *   - chosenUnlocks wiring: R-CHOSEN (AC 16)
 *   - spendUnlockToken action: A1–A4 (AC 18)
 *   - ensureProfile backfill: B1, B2 (AC 17)
 *
 * Standalone file (not extending streak-view.test.ts) to isolate profile state
 * without risking Story 2.3's R1–R8 tests. See deferred decision DD-2-9-Step4-3.
 *
 * Verified semantic summary (from AC 16):
 ...
 * Post-Story-2.9, the wiring always passes an array (never undefined) once profile is loaded.
 */
...
// ==========================================================================
// R-CHOSEN: chosenUnlocks wiring (AC 16)
// ==========================================================================

describe('chosenUnlocks wiring', () => {
  it('returns [] (not passive-map) when unlockedThemes: [] (R-CHOSEN initial)', () => {
  ...
  it('reflects chosen theme after profile mutation (R-CHOSEN reactive)', () => {
    seedStreakData(30, '2026-04-05')
  ...
  it('passes unknown strings verbatim without crashing (JSONB tolerance, AC 25)', () => {
...
// ==========================================================================
// A1–A4: spendUnlockToken action (AC 18)
// ==========================================================================

describe('spendUnlockToken action', () => {
  it('adds theme to array (A1)', () => {
    store$.profile.set({
      word_goal: 750,
      themeName: 'ink',
      customTheme: null,
  ...
  it('is idempotent — no duplicate on second call (A2)', () => {
    store$.profile.set({
      word_goal: 750,
      themeName: 'ink',
      customTheme: null,
  ...
  it('appends without overwriting existing entry (A3)', () => {
    store$.profile.set({
      word_goal: 750,
      themeName: 'ink',
      customTheme: null,
  ...
  it('calls ensureProfile for null profile (A4)', () => {
...
// ==========================================================================
// B1–B2: ensureProfile backfill (AC 17)
// ==========================================================================

describe('ensureProfile backfill', () => {
  it('backfills [] on legacy profile with editor but no unlockedThemes (B1)', () => {
    // Simulate a persisted profile from before Story 2.9 — has editor but no unlockedThemes
    store$.profile.set({
      word_goal: 750,
      themeName: 'ink',
      customTheme: null,
  ...
  it('includes unlockedThemes: [] on fresh profile (B2)', () => {
    expect(store$.profile.get()).toBeNull()
