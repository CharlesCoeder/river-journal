...
beforeEach(() => {
  batch(() => {
    ...
    flows$.set({})
    graceDays$.set({})
    // Reset session so per-test setup of store$.session.userId is deterministic
    // (cross-user defense in store$.views.streak scopes by current userId).
...
describe('store$.views.streak — reactive computed view (AC13)', () => {
  ...
  it('R5: E2E reactive cycle — graceDays$ write bridges a gap and extends currentStreak', () => {
    ...
    // The streak view scopes inputs by session.userId (cross-user defense).
    // The grace-day fixture below uses userId='user-1', so set the session
    // to match — and stamp the entries/flows with the same id so they survive
    // the scoped filter. (Pre-defense, this test ran with anonymous fixtures
    // and no session; the defense rejects the cross-id mix.)
    store$.session.userId.set('user-1')
    batch(() => {
      entries$.set({
        [eToday]: mkEntry(eToday, today, { user_id: 'user-1' }),
        [eTwo]: mkEntry(eTwo, ydayMinus2, { user_id: 'user-1' }),
      })
      flows$.set({
        [fToday]: mkFlow(fToday, eToday, 500, { user_id: 'user-1' }),
        [fTwo]: mkFlow(fTwo, eTwo, 500, { user_id: 'user-1' }),
      })
    })
...
// ─────────────────────────────────────────────────────────────────────────────
// Cross-user defense — streak input scoping
// (spec-fix-cross-user-data-defense — I/O matrix rows 12, 13)
// ─────────────────────────────────────────────────────────────────────────────

describe('store$.views.streak — cross-user input scoping', () => {
  it('signed-in session: only current-user rows contribute; prior-user qualifying days are excluded', () => {
    const today = getTodayJournalDayString()
    const yday = shiftDay(today, -1)

    // Prior user (user-A) has two qualifying days; current user (user-B) has one.
    // Without scoping, currentStreak would inflate to 2 by counting A's day.
    store$.session.userId.set('user-B')
    batch(() => {
      entries$.set({
        'e-A-yday': mkEntry('e-A-yday', yday, { user_id: 'user-A' }),
        'e-B-today': mkEntry('e-B-today', today, { user_id: 'user-B' }),
      })
      flows$.set({
        'f-A-yday': mkFlow('f-A-yday', 'e-A-yday', 500, { user_id: 'user-A' }),
        'f-B-today': mkFlow('f-B-today', 'e-B-today', 500, { user_id: 'user-B' }),
      })
    })
  ...
  it('signed-in session: B-owned flow under A-owned parent is dropped (parent-membership scoping)', () => {
    const today = getTodayJournalDayString()
    store$.session.userId.set('user-B')
    batch(() => {
      entries$.set({ 'e-A': mkEntry('e-A', today, { user_id: 'user-A' }) })
      flows$.set({ 'f-B-under-A': mkFlow('f-B-under-A', 'e-A', 500, { user_id: 'user-B' }) })
    })

    // Parent entry belongs to A → not in scoped entries → flow dropped → no streak.
    expect(store$.views?.streak?.get()?.currentStreak).toBe(0)
  })

  it("signed-in session: prior-user grace days don't bridge current-user gaps", () => {
    const today = getTodayJournalDayString()
    const ydayMinus1 = shiftDay(today, -1)
    const ydayMinus2 = shiftDay(today, -2)
    ...
    batch(() => {
      entries$.set({
        'e-B-today': mkEntry('e-B-today', today, { user_id: 'user-B' }),
        'e-B-two': mkEntry('e-B-two', ydayMinus2, { user_id: 'user-B' }),
      })
      flows$.set({
        'f-B-today': mkFlow('f-B-today', 'e-B-today', 500, { user_id: 'user-B' }),
        'f-B-two': mkFlow('f-B-two', 'e-B-two', 500, { user_id: 'user-B' }),
      })
      // Grace day is owned by user-A → must NOT bridge user-B's gap at today-1.
      graceDays$.set({ 'g-A': mkGrace('g-A', ydayMinus1, { userId: 'user-A' }) })
    })

    expect(store$.views?.streak?.get()?.currentStreak).toBe(1)
  })

  it('anonymous session (signed-out): only user_id===null rows contribute; foreign rows ignored', () => {
    const today = getTodayJournalDayString()
    store$.session.userId.set(null)
    batch(() => {
      entries$.set({
        'e-anon': mkEntry('e-anon', today), // user_id undefined → anonymous
        'e-A-today': mkEntry('e-A-today', today, { user_id: 'user-A' }),
      })
      flows$.set({
        'f-anon': mkFlow('f-anon', 'e-anon', 500),
        'f-A-today': mkFlow('f-A-today', 'e-A-today', 500, { user_id: 'user-A' }),
      })
    })

    // Anonymous-only scope qualifies today exactly once via the orphan flow.
    expect(store$.views?.streak?.get()?.currentStreak).toBe(1)
  })
})
