...
//
// ---------------------------------------------------------------------------
// Second extension for server-side PostHog emission: on the path where
// fanOutExpoPush actually runs (opted-in recipient with live tokens), the
// handler now calls `emitServerEvent('moderation_notification_delivered',
// SERVER_DISTINCT_ID, { action_type: payload.action_type, sent_count,
// failed_count })` derived from `result` at the final logInfo call, awaited
// before the final `return ok()`. It does NOT emit on the dedupe no-op, the
// add_note no-op, a no-target resolution, an opted-out/unknown preference
// state, or a token-lookup failure -- no delivery happened on any of those
// paths. As with notify_reply, no handler signature change is needed:
// emitServerEvent's fetch defaults to `globalThis.fetch` (the same global the
// existing Expo-fanout tests already stub), gated on POSTHOG_API_KEY -- every
// pre-existing test above leaves that env var unset, so this extension is a
// silent no-op for them. The new tests below set POSTHOG_API_KEY and route
// the global fetch stub by URL (Expo vs the PostHog capture endpoint).
//
...
// fails at module resolution before ANY test in this file runs.
...
// ---------------------------------------------------------------------------
// Server-side PostHog emission -- moderation_notification_delivered fires
// ONLY on the path where fanOutExpoPush actually runs (enabled + tokens
// present), never on the dedupe/add_note/no-target/opted-out/token-error
// no-op paths.
// ---------------------------------------------------------------------------
...
// Routes the stubbed global fetch by URL, mirroring notify_reply's
// withRoutedFetch: an Expo push POST (exp.host) vs a PostHog capture POST
// (any other URL) -- a real delivery run now issues both.
...
Deno.test('handler emits moderation_notification_delivered with the derived metadata + SERVER_DISTINCT_ID after a real delivery attempt', async () => {
  await withServiceRoleKey(SERVICE_ROLE_KEY, async () => {
    await withPosthogApiKey('phc_test_key', async () => {
      await withRoutedFetch(async (calls) => {
        const client = buildFullMockClient('00000000-0000-0000-0000-0000000000b1', {
          ...
          tokens: {
            data: [{ user_id: AFFECTED_USER_ID, expo_push_token: 'ExponentPushToken[mod-2]' }],
            error: null,
          },
        })
        const response = await handler(moderationRequest(suspendUserPayload()), client)
        ...
        assertEquals(body.event, 'moderation_notification_delivered')
        assertEquals(body.distinct_id, SERVER_DISTINCT_ID)
        assertEquals(body.properties, {
          action_type: 'suspend_user',
          sent_count: 1,
          failed_count: 0,
        })
...
Deno.test('handler does NOT call the PostHog capture endpoint on the dedupe short-circuit', async () => {
  await withServiceRoleKey(SERVICE_ROLE_KEY, async () => {
    await withPosthogApiKey('phc_test_key', async () => {
      await withRoutedFetch(async (calls) => {
        const client = {
...
Deno.test('handler does NOT call the PostHog capture endpoint for an add_note action (no-op before the ledger claim)', async () => {
  await withServiceRoleKey(SERVICE_ROLE_KEY, async () => {
    await withPosthogApiKey('phc_test_key', async () => {
      await withRoutedFetch(async (calls) => {
        const client = {
          from(table: string) {
            throw new Error(`unexpected table access ("${table}") for an add_note no-op`)
          },
          // deno-lint-ignore no-explicit-any
        } as any
        const response = await handler(
...
Deno.test('handler does NOT call the PostHog capture endpoint when the affected user cannot be resolved (no-target)', async () => {
  await withServiceRoleKey(SERVICE_ROLE_KEY, async () => {
    await withPosthogApiKey('phc_test_key', async () => {
      await withRoutedFetch(async (calls) => {
        const client = buildFullMockClient('00000000-0000-0000-0000-0000000000b1', {
...
Deno.test('handler does NOT call the PostHog capture endpoint when the affected user has opted out (moderation.enabled false)', async () => {
  await withServiceRoleKey(SERVICE_ROLE_KEY, async () => {
    await withPosthogApiKey('phc_test_key', async () => {
      await withRoutedFetch(async (calls) => {
        const client = buildFullMockClient('00000000-0000-0000-0000-0000000000b1', {
...
Deno.test('handler does NOT call the PostHog capture endpoint when the token lookup errors', async () => {
  await withServiceRoleKey(SERVICE_ROLE_KEY, async () => {
    await withPosthogApiKey('phc_test_key', async () => {
      await withRoutedFetch(async (calls) => {
        const client = buildFullMockClient('00000000-0000-0000-0000-0000000000b1', {
          ...
          tokens: { data: null, error: { message: 'connection reset' } },
        })
        const response = await handler(moderationRequest(suspendUserPayload()), client)
        assertEquals(response.status, 200)
        assertEquals(calls.posthog.length, 0)
