...
test('Next.js build completes', async () => {
  try {
    // Use prebuild instead of build to avoid the expensive Tauri Rust compilation
    // This tests the Next.js build process which is what we care about for CI/CD
    buildProcess = exec('yarn prebuild', {
